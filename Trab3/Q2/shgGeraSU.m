function [shg, w] = shgGeraSU(nen,nint,Beta)
  %define pontos de integração e pesos
  switch (nint)
    %grau 1, 2 pts de integraçWão
    case 2
      pt(1) = -sqrt(3)/3;
      pt(2) = sqrt(3)/3;
      w(1) = 1;
  ...
  for l = 1:nint
    ...
    switch (nen)
      %grau 1
      case 2
        shg(1,1,l) = 0.5*(1.0 - t) - 3*Beta*(1-t^2)/4;
        shg(1,2,l) = 0.5*(1.0 + t) + 3*Beta*(1-t^2)/4;
        shg(2,1,l) = -1.0/2.0 + 3.0*t*Beta/2;
        shg(2,2,l) = 1.0/2.0 - 3.0*t*Beta/2;;
      %grau 2
      case 3
